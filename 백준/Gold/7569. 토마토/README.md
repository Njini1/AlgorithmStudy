...
- 어렵지는 않았으나 상, 하, 좌, 우 와 앞, 뒤를 2개의 배열로 나눠서 품. → 하나의 배열로 관리하는게 효율적임
...
- 익은 토마토는 큐(tomatoQ)에 넣고 안익은 토마토는 zeroCnt + 1을 함
- tomatoQ가 비어있어 있을 경우 익은 토마토가 없기 때문에 -1을 출력
1. 한 번 돌때마다 큐의 크기를 측정하여 해당 크기만큼만 탐색하고 answer + 1
    1. 큐에서 하나씩 뽑아 상하좌우앞뒤를 모두 탐색하여 안익은 토마토를 익은 토마토로 바꾸고 zeroCnt - 1을 함. 해당 data의 값도 1로 바꿈
2. 큐에 있는 토마토를 모두 탐색한 뒤에도 zeroCnt가 0이 아니라면 모두 익게 하지는 못하는 상태이므로 -1을 출력하고 모두 익었다면 answer를 출력.
