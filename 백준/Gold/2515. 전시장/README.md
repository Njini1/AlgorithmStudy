...
- DP까지는 생각했지만 해당 값을 선택하고 어떤 dp의 값에 더해야 할 지를 생각못함.
- 또한 가능한 DP값을 완전탐색하면 시간 초과날 거 같았음. → 이진 탐색을 생각못함..
...
- **정렬**: `height` 기준 정렬 필요
- **이분 탐색**: 현재 그림과 높이 차이가 `K 이상` 나는 가장 가까운 그림을 빠르게 찾기 위해 사용
- **DP 배열**: `dp[i]`는 i번째 그림까지 고려했을 때 가능한 최대 가격합을 저장
    - 현재 값을 선택한 경우와 선택하지 않은 경우 고려
