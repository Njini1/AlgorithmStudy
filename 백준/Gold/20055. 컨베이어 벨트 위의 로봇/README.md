...
- 1부터 N까지의 레일에서만 로봇이 움직인다는 것
- 내리는 위치(downIdx)에서는 즉시 로봇이 내림

### 풀이 방법

- 컨베이어가 회전할 때마다 배열의 값을 바꾸는 것보다 올리는 위치의 인덱스(upIdx)와 내리는 위치 인덱스(downIdx)를 조절함
1. upIdx와 downIdx를 조절하여 회전
2. 회전 후 downIdx의 자리에 로봇이 있다면 즉시 로봇을 내림(robotsLocation에서 downIdx의 값이 true라면 false로 변경)
3.  컨베이어 벨트의 첫번째 라인(1부터 N번)에 있는 로봇을 탐색. 해당 칸에 로봇이 있고 다음칸에 로봇이 없고 내구도가 0보다 크다면 내구도를 -1 한 후 이동시킴. -1 후 0이라면 zeroCnt를 + 1함.
robotsLocation의 현재 칸은 false로, 다음칸은 true로 변경.
4. 올리는 위치(upIdx)에 로봇이 없고 내구도가 0이라면 새로운 로봇을 올림.
5. 단계(step)을 + 1함.
