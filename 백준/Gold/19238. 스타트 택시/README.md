...
- 문제는 어렵지 않았으나 예외적인 부분을 잡는다고 오래 걸림.
- “모든 출발지와 목적지는 빈칸이고, 모든 출발지는 서로 다르며, 각 손님의 출발지와 목적지는 다르다.”
→ 현 승객의 출발지와 다른 승객의 목적지가 같을 수 있다
...
1. 손님의 출발지는 map에 number 저장, 목적지는 따로 목적지 배열에 저장
2. 손님 태우기
    1. 현재 위치와 손님이 같은 위치에 있다면 바로 손님 데려다 주기
    2. 아니라면 최단 경로인 승객 태우기(findPoint)
        1. BFS로 탐색하면서 승객을 찾는다면 우선순위 큐에 넣음
            1. 빈 큐가 아니라면 그 중 행이 작고 같으면 열이 작은 것을 하나 빼서 해당 승객을 태움. targetNum에 해당 번호를 넣음.
            2. 빈 큐라면 승객을 찾을 수 없기 때문에 -1 반환
3. 손님 데려다 주기
    1. 아직 연료가 남아있고 반환된 이동 거리(cnt)가 -1이 아니라면 BFS로 탐색하면서 해당 손님의 목적지까지 데려다줌.
