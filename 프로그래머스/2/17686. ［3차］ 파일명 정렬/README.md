...
- 정규표현식과 정렬을 써야한다고 생각은 했지만 제대로 사용할 줄은 몰라서 다시 공부
...
- 정규 표현식 사용
    - 첫번째 단어: 영어 대소문자, “.”, “-”, 공백 가능
    ⇒ `([a-zA-Z .-]+)` : 대괄호 안에 있는 것은 들어올 수 있는 문자 의미, “+”는 한번 이상 반복 의미, 소괄호는 그룹 의미
    - 두번째 단어: 숫자만 가능
    => `(\\d+)` : 숫자가 하나 이상 있음.
    - 세번째 단어: 영어 대소문자, “.”, “-”, 공백 가능
    ⇒ `([a-zA-Z0-9 .-]*)?` : 해당 문자들이 없을 수도 있고 1개 이상일 수도 있음. 그리고 이 그룹은 아예 없을 수도 있고 한 개만 있을 수 있음.
- 주어진 문자열을 해당 3부분으로 나누고 세번째 문자가 없다면 null 저장
- 첫번째 문자를 기준으로 정렬하고 이 값이 같다면 두번째 문자를 기준으로 문자가 5개 이상이면 5개까지만 잘라서 숫자로 만든 후 비교 후 정렬.
- 정렬한 값들을 다시 합쳐서 출력
