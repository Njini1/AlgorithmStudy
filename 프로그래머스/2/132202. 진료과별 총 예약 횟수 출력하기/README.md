...
### 풀이
- SUBSTRING(문자열, 시작 위치, 개수), LEFT(문자열, 개수)
...
- 컬럼명은 '진료과코드', '5월예약건수'로 지정
    - 별칭(alias)를 사용한다. (띄어쓰기 없으면 따옴표 생략가능)
    - **AS 별칭** 형태로 써주는데 별칭에 띄어쓰기가 있는 경우에는 따옴표를 꼭 붙여줘야 한다.
- 정렬
    - ORDER BY 후 '5월예약건수', '진료과코드' 이런 식으로 써주면 안된다. 
    GROUP BY, ORDER BY, HAVING 절에 별칭을 쓸 경우,  `별칭`과 같이을 붙여서 써줘야 한다. '별칭'(작은따옴표)은 문자 그대로 별칭에 적힌 문자로 ORDER BY를 해주겠다는 의미가 되기 때문이다. (너무 헷갈리면 그냥 ORDER BY 2, 1과 같이 SELECT문의 컬럼명 순서로 써주는 것도 방법)
