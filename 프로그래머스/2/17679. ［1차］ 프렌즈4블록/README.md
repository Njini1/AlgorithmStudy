...
- 그냥 구현만 잘하면 되었기에 어렵지는 않았으나 코드가 더러워져서 이게 맞나…? 의문이 들면서 풀었음.
...
1. 삭제할 수 있는 블록이 있을 때까지 계속 findBlock함수 호출
    1. findBlock 의 결과값이 true라면 삭제할 블록이 있다는 의미이기 때문에 removeBlock 함수 호출
    2. false라면 삭제할 수 있는 블록이 없다는 의미이므로 종료 후 결과값 출력
   
- findBlock 함수 → 2×2 블록 찾기
    - `removeBlock = fasle;` 세팅 → 지울 수 있는 블록이 있는지 저장
    - 2중 for문으로 board의 값을 가로 2칸, 세로2칸씩 블록을 확인하면서 빈 공간이 아니면서(”-”) 4칸이 모두 같은 값이라면 `removeBlock = true`로 변경. → 함수는 true 반환
    - 지울 수 있는 값이 아니라면 계속 다음 위치 탐색. 하나도 지울 수 있는 블록이 없다면 함수 false 반환
- removeBlock → 블록 삭제 및 아래로 내리기
    - 기존 블록 중 살아남은 블록만 리스트에 저장
    - 해당 블록들을 for문을 사용해서 리스트의 값들을 아래쪽부터 채우고, 남은 공간은 '-'으로 저장
