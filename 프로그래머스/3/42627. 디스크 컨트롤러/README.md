...
- 효율적인 코드가 있겠으나 비효율적인 코드밖에 생각안나서 그렇게 풀었으나 그것 또한 제대로 안됨.
- 블로그 참고
...
- 시간(t)을 0부터 1초씩 흐르게 한다.
    - `for (int t = 0; t <= 1000; t++)` 처럼 매 초마다 체크.
- 현재 시간(t)에 요청된 작업이 있다면,
    
    → 대기 큐(waitQ) 에 넣는다.
    
- 대기 큐가 비어있지 않다면,
    - 이전 작업(preEndTime)이 끝났는지 체크하고,
    - 끝났으면 대기 큐에서 가장 짧은 소요시간을 가진 작업을 꺼낸다.
- 현재 작업을 처리한다.
    - `preEndTime`에 소요시간을 더하고,
    - 요청부터 끝날 때까지 걸린 시간(=끝난 시간 - 요청 시간)을 누적한다.
- 대기 큐가 비어있고, 모든 작업을 다 큐에 넣었다면 종료.
    - `if (jobIdx == jobs.length)`로 체크.
