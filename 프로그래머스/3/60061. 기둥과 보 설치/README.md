...
- 원래는 설치하고 삭제한 곳만 제대로 설치되었는 지 확인하는 코드로 짰는데 예외가 있었음.
- 그래서 설치는 설치한 곳만 확인하면 되었지만 삭제는 아니었음
- 삭제는 삭제 후 현재까지 설치된 곳을 모두 확인하는 방법과, 삭제된 사방만 탐색하는 방법이 있었음.
- 그리고 <실패 코드1>을 보면 y의 값을 들어오는 값에서 n - y하면서 불필요한 계산을 함 → 기둥과 보를 배열 아래에 설치, 삭제하고 싶었기 때문
⇒ 근데 그냥 위에 설치, 삭제하는 것으로 하는게 훨씬 편했음.
- 또한 위치 확인 시 x -1, y - 1을 하는데 범위 확인도 안했어서 추가함.
...
- int[][][] map에 기둥, 보 설치 저장
- 기둥 설치, 보 설치, 기둥/보 삭제로 나눔
- 기둥 설치 시 `checkBuildPillar()` 호출 → 기둥 설치 기준 확인. 맞다면 true 반환 → 설치 가능하다면 `map[y][x][0] = true;`
- 보 설치 시 `checkBuildPillar()` 호출 → 보 설치 기준 확인. 맞다면 true 반환 → 설치 가능하다면 `map[y][x][1] = true;`
- 기둥, 보 삭제 시 `canDelete()` 호출 → 해당 값 삭제 후 2중 for문으로 하나씩 다 확인.

### 시간 복잡도

- 시간 복잡도는 최악 O(1000 × 1000 × 2)
