...
### 공부
- group, having
- `HAVING`을 사용한 이유는 집계 함수(`SUM`, `AVG`, `COUNT` 등)의 결과를 필터링하기 위해서
    - **WHERE**: **집계 전**에 데이터를 필터링, 개별 행에 조건을 걸 때
    - **HAVING**: **집계 후**에 그룹의 결과를 필터링, group by 후 집계값에 조건을 걸 때
